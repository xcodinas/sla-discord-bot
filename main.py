...
def check_sidebar(image_path):
    ...
    result = cv2.matchTemplate(gray_image, sidebar_template, cv2.TM_CCOEFF_NORMED)
    ...
    extracted_text = translator.translate(pytesseract.image_to_string(thresh_roi), src='auto', dest='en').text
...
def extract_numbers_with_template(image_path, template_path):
    ...
    if sidebar:
        ...
        gray_image = gray_image[:, :gray_image.shape[1] - (sidebar_width + 300)]
    ...
    roi = thresh[top_left[1]:bottom_right[1],
          top_left[0]:bottom_right[0] + 150]  # Extend width to include the numbers
...
# TODO: ADD tests optimize waith chatgpt
